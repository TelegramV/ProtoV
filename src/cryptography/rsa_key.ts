import BigInteger from "big-integer";
import {fromHex, toHex} from "../utils/bytes";
import {SHA1} from "./hash";

type Key = {
    modulus: string;
    exponent: string;
}

const keys: Key[] = [
    {
        modulus: "",
        exponent: "010001"
    },
    {
        modulus: "",
        exponent: "010001"
    },
    {
        modulus: "",
        exponent: "010001"
    },
    {
        modulus: "",
        exponent: "010001"
    },
    {
        modulus: "",
        exponent: "010001"
    },
];

const parsedKeys: { [prop: string]: Key; } = {};

function writeBuffer(array: Uint8Array, buffer: Buffer, offset: number): number {
    const length = array.length;

    if (length <= 253) {
        buffer.writeUInt8(length, offset++);
    } else {
        buffer.writeUInt8(254, offset++);
        buffer.writeUInt8(length & 0xFF, offset++);
        buffer.writeUInt8((length & 0xFF00) >> 8, offset++);
        buffer.writeUInt8((length & 0xFF0000) >> 16, offset++);
    }

    buffer.set(array, offset);
    offset += length;

    while (offset % 4) {
        buffer.writeUInt8(0, offset++);
    }

    return offset;
}

for (let i = 0; i < keys.length; i++) {
    const key: { modulus: string; exponent: string; } = keys[i];

    const modulusBuffer = fromHex(key.modulus);
    const exponentBuffer = fromHex(key.exponent);

    const buffer = new Buffer(modulusBuffer.length + exponentBuffer.length + 8);
    let offset = 0;

    offset = writeBuffer(modulusBuffer, buffer, offset);
    offset = writeBuffer(exponentBuffer, buffer, offset);

    const fingerprint = SHA1(buffer.slice(0, offset)).slice(-8).reverse();

    parsedKeys[toHex(fingerprint)] = {
        modulus: key.modulus,
        exponent: key.exponent
    };
}

function rsa_key(fingerprints: string[]): void | Key & { fingerprint: string; } {
    for (let i = 0; i < fingerprints.length; i++) {
        const key = parsedKeys[BigInteger(fingerprints[i]).toString(16)];

        if (key) {
            return {...key, fingerprint: fingerprints[i]};
        }
    }
}

export default rsa_key;